...
class ReconstructionRunner:
    ...
    def run(self):
        ...
        for epoch in range(self.startepoch, self.nepochs + 1):
            ...
            nonmnfld_pnts = self.sampler.get_points(mnfld_pnts.unsqueeze(0), mnfld_sigma.unsqueeze(0))
            # nonmnfld_pnts, empty_pnts = self.sampler.get_points(mnfld_pnts.unsqueeze(0), mnfld_sigma.unsqueeze(0), self.is_empty_check,
            # self.size)
            nonmnfld_pnts = nonmnfld_pnts.squeeze()
            # empty_pnts = empty_pnts.squeeze()
            ...
            if self.with_normals:
                ...
                # else:
                non_surface_normal_loss = non_surface_normal_loss_1 * 0
            ...
            self.writer.add_scalar("training/loss_outside", 0.0, epoch)
    ...
    def __init__(self, **kwargs):
        ...
        self.is_empty_check = self.get_empty_index(self.empty_data, kwargs["size"])
    ...
    def get_empty_index(self, empty_pts, size):
        is_empty_array = np.zeros([
            (size + 2) * (size + 2) * (size + 2),
        ])
        emtpy_index = ((empty_pts + (1 + 2 / (1 * size))) * size / 2).astype(int)
        emtpy_index = emtpy_index[:, 0] * ((size + 2)**2) + emtpy_index[:, 1] * (size + 2) + emtpy_index[:, 2]
        is_empty_array[emtpy_index] = True
        # print(emtpy_index[:10,])

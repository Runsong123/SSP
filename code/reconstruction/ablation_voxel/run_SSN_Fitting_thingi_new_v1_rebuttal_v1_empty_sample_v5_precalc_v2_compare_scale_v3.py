...
from model.sample_SSN_uniform_v1 import Sampler
...
class ReconstructionRunner:
    ...
    def initial_sample_parameter(self):
        ...
        self.uniform_free_sample_num_every_grid = int(10 / ((self.scale_list[self.scale_id])**3))
        self.uniform_free_grid_num = self.points_batch // (9 * self.uniform_free_sample_num_every_grid)

        self.uniform_normal_free_sample_num_every_grid = int(10 / ((self.scale_list[self.scale_id])**3))
        self.uniform_normal_free_grid_num = self.points_batch // (9 * self.uniform_normal_free_sample_num_every_grid)

        self.per_grid_num = int(10 / ((self.scale_list[self.scale_id])**3))
        self.empty_points_num = self.points_batch // (self.per_grid_num)
    ...
    def loss_for_empty_value(self, outside_loss, empty_indices):
        outside_loss_numpy = (outside_loss).detach().cpu().numpy().reshape(-1, self.per_grid_num)
    ...
    def loss_for_no_surface_value(self, non_surface_loss, uniform_indice_free):
        # print(n)
        uniform_free_space_loss = (non_surface_loss).detach().cpu().numpy().reshape(-1, self.per_grid_num)
    ...
    def loss_for_no_surface_normal(self, non_surface_normal_loss, uniform_indice_free):
        ...
        uniform_free_space_normal_loss = (non_surface_normal_loss.detach().cpu().numpy().reshape(-1, self.per_grid_num))
    ...
    def run(self):
        ...
        for epoch in range(self.startepoch, self.nepochs + 1):
            ...
            empty_pnts = self.sampler.get_points(empty_grid_pts_pns.unsqueeze(1), self.size, self.empty_points_num,
                                                 self.per_grid_num).squeeze()
            ...
            random_choice = np.random.randint(self.sampling_size,
                                              size=(size, self.per_grid_num)) + np.arange(size).reshape(-1, 1) * self.sampling_size

            nonmnfld_pnts_info = self.cal_data[uniform_indice_free].reshape(-1, 7)[random_choice.reshape(-1), :].reshape(
                -1, self.per_grid_num, 7)
    ...
    def __init__(self, **kwargs):
        ...
        self.scale_id = kwargs["scale_id"]
        # print(type(self.scale_id))
        # GPU settings
        ...
        if kwargs["dataset"] == "abc_noisefree" or kwargs["dataset"] == "famous_noisefree" or kwargs["dataset"] == "thingi":
            ...
            files_name = glob.glob("%s_%s_adaptive_more_grid11_density_boundary_depth_scale_all/%d/%s_grid_*.txt" % (
                "/research/d5/gds/rszhu22/SSN_Fitting_current/data/adaptive_more_grid/output_res_1",
                kwargs["dataset"],
                self.scale_id,
                kwargs["shape"],
            ))
            print(files_name)
            kwargs["size"] = int(files_name[0].split("_")[-3])
            self.size = kwargs["size"]
            self.grid_scale = 2.0 / self.size
            print(kwargs["size"])
            search_number = int(files_name[0].split("_")[-1].split(".")[0])
            self.random_sample_num_every_grid = search_number
            self.uniform_sample_num_every_grid = search_number
            self.uniform_normal_sample_num_every_grid = search_number
            self.empty_file = "%s_%s_adaptive_more_grid11_density_boundary_depth_scale_all/%d/%s_empty_%d_pts.txt" % (
                "/research/d5/gds/rszhu22/SSN_Fitting_current/data/adaptive_more_grid/output_res_1",
                kwargs["dataset"],
                self.scale_id,
                kwargs["shape"],
            ...
            self.surface_grid_file = "%s_%s_adaptive_more_grid11_density_boundary_depth_scale_all/%d/%s_grid_%d_pts_%d.txt" % (
                "/research/d5/gds/rszhu22/SSN_Fitting_current/data/adaptive_more_grid/output_res_1",
                kwargs["dataset"],
                self.scale_id,
                kwargs["shape"],
            ...
            self.other_grid_file = "%s_%s_adaptive_more_grid11_density_boundary_depth_scale_all/%d/%s_other_%d_pts.txt" % (
                "/research/d5/gds/rszhu22/SSN_Fitting_current/data/adaptive_more_grid/output_res_1",
                kwargs["dataset"],
                self.scale_id,
                kwargs["shape"],
        ...
        self.scale_list = [0.5, 0.75, 1.5, 2]
        cache_dir = "/research/d5/gds/rszhu22/SSN_Fitting_current/code_v8/data_prepare/data_precompute_scale_voxel"
        # cache_dir = "/research/d5/gds/rszhu22/SSN_Fitting_current/code_v8/reconstruction/data_precompute"
        cache_file = "%s/%s_%d.npy" % (cache_dir, self.shape, self.scale_id)
        if os.path.exists(cache_file):
            self.cal_data = np.load(cache_file)
            self.sampling_size = self.cal_data.shape[1]
        else:
            start_time = time.time()
            self.cal_data = self.precalc(self.other_data, self.scale_id)
            self.sampling_size = self.cal_data.shape[1]
    ...
    def precalc(self, other_data, scale_id):
        scale = self.grid_scale
        grid_num = other_data.shape[0]
        print(grid_num)

        ablation_scale = self.scale_list[scale_id]
        # calc_pts = np.zeros([grid_num, 8000, 4])
        calc_pts = []

        # 0.5 0.2*0.5 = 0.1
        offset = np.mgrid[-1:1:(0.2 * ablation_scale), -1:1:(0.2 * ablation_scale), -1:1:(0.2 * ablation_scale)].reshape(3, -1).T
        sampling_size = offset.shape[0]
        ...
        for p in np.array_split(other_data, 5, axis=0):
            pts = np.expand_dims(p, axis=1) + offset.reshape(1, sampling_size, 3)
            pts = pts.reshape(-1, 3)
            d = self.ptree.query(pts, 1, workers=-1)
            index = d[1][:].reshape(-1)
            calc_pts.append(d[0][:].reshape(-1, sampling_size))
            corres_normal.append(self.data[index, 3:].reshape(-1, sampling_size, 3))
            # print(i)
            # i += 1
        corres_normal = np.concatenate(corres_normal, axis=0).reshape(-1, sampling_size, 3)
        calc_pts = np.concatenate(calc_pts, axis=0).reshape(-1, sampling_size, 1)
        offset_pts = np.expand_dims(other_data, axis=1) + offset.reshape(-1, sampling_size, 3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conf", type=str, default="confs/setup_base_all_PCA_with_pe_v1.conf")
    ...
    parser.add_argument("--scale_id", type=int, default=-1, help="scale_size")

...
class Sampler(metaclass=abc.ABCMeta):
    ...
    def get_sampler(sampler_type):
        ...
        return utils.get_class("model.sample_SSN_uniform.{0}".format(sampler_type))
...
class NormalPerPoint(Sampler):
    ...
    def get_points(
        ...
        offset = torch.from_numpy(np.random.uniform(-1 * scale/2 , scale/2+1e-9, uniform_free_grid_num* 10 * dim).reshape(uniform_free_grid_num,10,dim))

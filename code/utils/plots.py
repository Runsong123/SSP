...
def plot_surface(
    ...
    if not os.path.exists(filename) or overwrite:
        ...
        layout = go.Layout(
            ...
            height=1200,
            scene=dict(
                xaxis=dict(range=[-1.05, 1.05], autorange=False),
...
def plot_cuts_axis(points, decoder, path, epoch, near_zero, axis, file_name_sep="/", latent=None):
    ...
    for index, pos in enumerate(position_cut):
        ...
        if near_zero:
            ...
            trace1 = go.Contour(
                ...
                autocontour=False,
                contours=dict(start=start, end=0.1, size=0.01)
                # ),colorbar = {'dtick': 0.05}
            )
        else:
            trace1 = go.Contour(
...
def plot_cuts(points, decoder, path, epoch, near_zero, latent=None):
    ...
    position_cut = np.vstack(([xx, np.zeros(xx.shape[0]), yy]))
    position_cut = [position_cut + np.array([0.0, i, 0.0]).reshape(-1, 1) for i in np.linspace(min_y - 0.1, max_y + 0.1, 10)]
    for index, pos in enumerate(position_cut):
        # fig = tools.make_subplots(rows=1, cols=1)

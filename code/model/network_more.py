...
class ImplicitNet(nn.Module):
    def __init__(self, d_in, dims, skip_in=(), geometric_init=True, radius_init=1, beta=100):
        ...
        lin = nn.Linear(dims[1], dims[1])
        torch.nn.init.constant_(lin.bias, 0.0)
        # torch.nn.init.eye_(lin.weight)
        lin.weight = nn.Parameter(torch.normal(0, std=0.000001, size=(dims[1], dims[1])) + torch.eye(dims[1]))
        setattr(self, "lin" + "midden", lin)

...
class Sampler(metaclass=abc.ABCMeta):
    ...
    def get_sampler(sampler_type):
        ...
        return utils.get_class("model.sample_SSN_v1.{0}".format(sampler_type))
...
class NormalPerPoint(Sampler):
    ...
    def get_near_points(self, pc_input, local_sigma=None):
        ...
        # sample_global = (
        #     torch.rand(batch_size, sample_size // 8, dim, device=pc_input.device) * (self.global_sigma * 2)
        # ) - self.global_sigma

        # sample = [sample_local, sample_global]
    ...
    def get_points(
        ...
        random_noise = torch.cat(
            [
                ...
                torch.randn([uniform_free_grid_num, 4, dim]) * scale / 2,
            ],
            dim=1,
        )

...
for shape_item in shape_list:
  ...
  max_L1_dis = np.max(np.abs(np.asarray(mesh.vertices) - mean_pts))
  max_dis = np.linalg.norm((np.asarray(mesh.vertices) - mean_pts), ord=2, axis=1).max()
  scale_mean_dict[shape_item] = [mean_pts, max_dis, max_L1_dis]

# scale_ouput_dir = "/research/dept6/khhui/SSN_Fitting/code/code_v8/reconstruction/evaluate/scaled_challenging_data/"
scale_ouput_dir = "/research/d5/gds/rszhu22/SSN_Fitting_current/code/reconstruction/evaluate/scaled_challenging_data_midden/"
...
# epoch_list = [3000,6000]
# shape_list = ["451676"]
# for epoch in epoch_list:
#     base_dir = "/research/d5/gds/rszhu22/DiGS/surface_reconstruction/log/surface_reconstruction/DiGS_surf_recon_experiment/result_meshes_check_midden/"
#     method_name = "DiGS_official_epoch_midden"
#     my_mkdir(os.path.join(scale_ouput_dir, method_name))
#     # shape_list = ["120477", "451676", "90276", "thingi_35269", "thingi_36371"]
#     missing_shape_list = []
#     for shape_item in shape_list:
#         mesh = trimesh.load(base_dir + "%s_%d.ply" % (shape_item, epoch))
#         mesh.vertices *= 1.1
#         mesh.export(scale_ouput_dir + "%s/%s_%d.ply" % (method_name, shape_item, epoch))


# base_dir = "/research/d5/gds/rszhu22/SSN_Fitting_current/code_v8/reconstruction/challenging/"
# method_name = "SSNFitting_CVPR_10000_EPOCH_midden"
# my_mkdir(scale_ouput_dir + method_name)
# epoch_list = [3000, 5000, 7000, 9000]
# shape_list = ["451676"]
# for epoch in epoch_list:
#     for shape_item in shape_list:
#         mesh = trimesh.load(base_dir + "igr_%d_%s.ply" % (epoch,shape_item))
#         mean_pts, max_dis, max_L1_dis = scale_mean_dict[shape_item]
#         # mesh.vertices *= max_L1_dis
#         # mesh.vertices /= max_dis
#         mesh.vertices *= 1.1
#         mesh.export(scale_ouput_dir + "%s/%s_%d.obj" % (method_name, shape_item, epoch))


# /research/d5/gds/rszhu22/SSN_Fitting_current/code_v8/empty_uniform_v5_precalc_v2/

base_dir = "/research/d5/gds/rszhu22/SAL/code/original_SAL_Challenging_data/thingi/451676/2022_10_24_19_35_47/plots/"
method_name = "SAL_10000_EPOCH_midden"
my_mkdir(scale_ouput_dir + method_name)
epoch_list = [2000, 4000, 6000, 8000, 10000, 12000, 14000]
shape_list = ["451676"]
for epoch in epoch_list:
    for shape_item in shape_list:
        mesh = trimesh.load(base_dir + "uniform_iteration_%d_0.html.ply" % (epoch))
        mean_pts, max_dis, max_L1_dis = scale_mean_dict[shape_item]
        mesh.vertices *= max_L1_dis
        mesh.vertices /= max_dis
        mesh.export(scale_ouput_dir + "%s/%s_%d.ply" % (method_name, shape_item, epoch))

base_dir = "/research/d5/gds/rszhu22/SAL/code/SAL_signed/thingi/451676/2022_11_10_21_48_02/plots/"
method_name = "SAL_signed_10000_EPOCH_midden"
my_mkdir(scale_ouput_dir + method_name)
epoch_list = [2000, 4000, 6000]
shape_list = ["451676"]
for epoch in epoch_list:
    for shape_item in shape_list:
        mesh = trimesh.load(base_dir + "uniform_iteration_%d_0.html.ply" % (epoch))
...
# G:\d5\gds\rszhu22\SAL\code\original_SAL_Challenging_data\thingi\451676\2022_10_24_19_35_47\plots

# /research/d5/gds/rszhu22/SAL/code/original_SAL_Challenging_data/thingi/451676/2022_10_24_19_35_47/plots
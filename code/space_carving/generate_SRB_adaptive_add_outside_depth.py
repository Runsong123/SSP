...
def normalize_save_abc(all_shape_list, all_data_input, all_data_normalized_save):
    for shape_name in all_shape_list:
        ...
        print(shape_name)
        pts = np.loadtxt("%s/%s.xyz"%(all_data_input ,shape_name))
        pts = normalize_input(pts)
...
def change_name(all_shape_list, grid_outdir):
    import os
    import shutil
    
    grid_outdir_new = grid_outdir+"_name_change"
    if not os.path.exists(grid_outdir_new):
        os.mkdir(grid_outdir_new)
    for shape_name in all_shape_list:
        file_item_name = glob.glob("%s/%s*_grid_*.txt"%(grid_outdir,shape_name))[0]
        size = int(file_item_name.split("_grid_")[1].split("_pts")[0])
        # size = 100
        shape_name_splits = shape_name.split(".")
        shape_name_new = shape_name_splits[0] + shape_name_splits[1]
        shutil.copy("%s/%s_grid_%d_pts.txt"%(grid_outdir, shape_name, size), "%s/%s_grid_%d_pts.txt"%(grid_outdir_new, shape_name_new, size))
        shutil.copy("%s/%s_empty_%d_pts.txt"%(grid_outdir, shape_name, size), "%s/%s_empty_%d_pts.txt"%(grid_outdir_new, shape_name_new, size))
        shutil.copy("%s/%s_other_%d_pts.txt"%(grid_outdir, shape_name, size), "%s/%s_other_%d_pts.txt"%(grid_outdir_new, shape_name_new, size))
     

if __name__ == "__main__":
    
    # all_type = ["famous_dense", "famous_original", "famous_extra_noisy", "famous_noisefree", "famous_original", "famous_sparse"]
    # all_type = ["low_noise", "med_noise", "high_noise"]
    all_type = ["deep_geometric_prior_data"]
    # E:\data\raw_data\optim_data\deep_geometric_prior_data
    # all_type = ["vardensity_gradient", "vardensity_striped"]
    # all_type = ["vardensity_striped"]
    makedir("deep_geometric_prior_data")
    for type_item in all_type:
        # list_file_path = "C:/Users/runsongzhu/Downloads/pclouds/testset_%s.txt"%type_item
        grid_outdir = "./output_res_1_pcp_%s_adaptive_more_grid11_max_100_density_boundary_depth"%type_item
        # grid_outdir = "output_res_1_pcpnet_%s_adaptive_more_grid_100"%type_item
        all_data_input = "E:/data/raw_data/optim_data/"
        # all_data_input = "C:/Users/runsongzhu/Downloads/pclouds//"%type_item
        all_data_normalized_save = "./deep_geometric_prior_data/%s"%type_item
        ...
        # if type_item[-5:]=="noise":
        #     change_name(all_shape_list,grid_outdir)

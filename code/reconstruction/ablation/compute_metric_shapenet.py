...
from model.network_with_pe import gradient
...
# Or set manually
# dataset_path = '/home/chamin/NSP_dataset/'
# raw_dataset_path = '/home/chamin/ShapeNetNSP'
# mesh_path = '/home/chamin/CVPR2022/DiBS/surface_reconstruction/log/shapenet_recon_siren_wo_n/DiGS_surf_recon_experiment'
raw_dataset_path = "/research/d5/gds/rszhu22/DiGS/data/ShapeNetNSP/"
dataset_path = "/research/d5/gds/rszhu22/DiGS/data/NSP_dataset/"
# Print metrics to file in logdir as well
os.makedirs(os.path.join("log"), exist_ok=True)
out_path = os.path.join("log", 'metric_summary.txt')
...
def get_network():
    confs_path = "confs/setup_base_all_PCA_with_pe_v1_shapenet.conf"
    conf_filename = "./reconstruction/" + confs_path
    conf = ConfigFactory.parse_file(conf_filename)
    # self.conf = ConfigFactory.parse_file(os.path.join(mesh_path, "config.conf"))

    d_in = conf.get_int("train.d_in")

    network = utils.get_class(conf.get_string("train.network_class"))(d_in=d_in,
...
for shape_class in order:
    ...
    base_dir = "/research/d5/gds/rszhu22/SSN_Fitting_current/code_v8/exps/shapenet/exps/"
    #  lamp_d2fc5882e1faa78c1592f292ab531da8/2023_05_31_09_42_52/evaluation
    results_meshes_path = glob.glob(f"{base_dir}/{shape_class}_*")

    chamfers[shape_class] = []
    IoUs[shape_class] = []

    for shape_exps in results_meshes_path:
        shape = shape_exps.split("/")[-1].split("_")[-1]
        

        timestamps = os.listdir(shape_exps)
        if (len(timestamps)) == 0:
            is_continue = False
            timestamp = None
        else:
            ...
            is_continue = True
        recon_shape_path = glob.glob(f"{shape_exps}/{timestamp}/evaluation/10000/*.ply")[0]
        # saved_weights_class_path = os.path.join(mesh_path, shape_class)

        saved_model_state = torch.load(os.path.join(f"{shape_exps}/{timestamp}", "checkpoints/ModelParameters", str(10000) + ".pth"))
        network = get_network()
        network.load_state_dict(saved_model_state["model_state_dict"])
        network.eval()
        network.set_mask(3 + 10000 // 1000)
        

        recon_mesh = trimesh.load(recon_shape_path)
        
        gt_shape_path = os.path.join(gt_shape_class_path, f"{shape}.ply")
        ...
        # DiGSNet.load_state_dict(torch.load(gt_shape_weights_path, map_location=device))
        # DiGSNet.to(device)

        cp = gt_pc.vertices.mean(0,keepdims=True)
        scale = np.max(np.linalg.norm(gt_pc.vertices - cp, axis=1)) * 1.1

        eval_points = (gen_points - cp) / scale
        eval_points = torch.tensor(eval_points, device=device, dtype=torch.float32)
        res = network(eval_points)
        ...
        # un normalize
        recon_mesh.vertices = recon_mesh.vertices * scale + cp
        # recon_mesh.export("check.ply")
        # np.savetxt("gt_points.txt", gt_points)
        # exit()
...
#         gt_shape_weights_path = os.path.join(saved_weights_class_path, shape_file.replace('.ply',''), 'trained_models', 'DiGS_model_0.pth')
#         gt_pc = trimesh.load(gt_shape_path)
#         gt_raw_shape_path = os.path.join(gt_raw_shape_class_path, shape)
#         points = np.load(os.path.join(gt_raw_shape_path, 'points.npz')) # [('points', (100000, 3)), ('occupancies', (12500,)), ('loc', (3,)), ('scale', ())]
#         pointcloud = np.load(os.path.join(gt_raw_shape_path, 'pointcloud.npz')) # [('points', (100000, 3)), ('normals', (100000, 3)), ('loc', (3,)), ('scale', ())]
...
#         eval_points = (gen_points - cp) / scale
#         eval_points = torch.tensor(eval_points, device=device, dtype=torch.float32)
#         res = DiGSNet.decoder(eval_points)
...
for key in order:
    ...
    # print("{} : Chamfers: {}\n Mean: {:e}, Median: {:e}, Std: {:e}".format(key, IoUs_np, IoUs_np.mean(), np.median(IoUs_np), IoUs_np.std()))
...
class ReconstructionRunner:
    ...
    def run(self):
        ...
        if self.eval:
            ...
            self.network.set_mask(3 + 10000 // 1000)
    ...
    def __init__(self, **kwargs):
        ...
        self.file_dict["thingi"] = "/optim_data/thingi_normalized_concate_adaFit/thingi/"
        self.file_dict["shapenet"] = "/shapenet_normal/"
        # config setting
        ...
        self.input_file = (self.conf.get_string("train.input_path") + self.file_dict[kwargs["dataset"]] + "/"+kwargs["class_name"] +"/" +kwargs["shape"] + ".npy")
        print(self.input_file)
        import glob

        if kwargs["dataset"] == "abc_noisefree" or kwargs["dataset"] == "famous_noisefree" or kwargs["dataset"] == "thingi" or kwargs["dataset"] == "shapenet":

            files_name = glob.glob("%s_%s_adaptive_more_grid11_density_boundary_depth/%s/%s_empty_*_pts.txt" % (
                "/research/d5/gds/rszhu22/SSN_Fitting_current/preprocess/output_res_1",
                kwargs["dataset"],
                kwargs["class_name"],
                kwargs["shape"],
            ))
            print("%s_%s_adaptive_more_grid11_density_boundary_depth/%s_empty_*_pts.txt" % (
                "/research/d5/gds/rszhu22/SSN_Fitting_current/preprocess/output_res_1",
                kwargs["dataset"],
                kwargs["shape"],
            ))
            ...
            print(kwargs["size"])
            self.empty_file = "%s_%s_adaptive_more_grid11_density_boundary_depth/%s/%s_empty_%d_pts.txt" % (
                "/research/d5/gds/rszhu22/SSN_Fitting_current/preprocess/output_res_1",
                kwargs["dataset"],
                kwargs["class_name"],
                kwargs["shape"],
            ...
            self.surface_grid_file = "%s_%s_adaptive_more_grid11_density_boundary_depth/%s/%s_grid_%d_pts.txt" % (
                "/research/d5/gds/rszhu22/SSN_Fitting_current/preprocess/output_res_1",
                kwargs["dataset"],
                kwargs["class_name"],
                kwargs["shape"],
            ...
            self.other_grid_file = "%s_%s_adaptive_more_grid11_density_boundary_depth/%s/%s_other_%d_pts.txt" % (
                "/research/d5/gds/rszhu22/SSN_Fitting_current/preprocess/output_res_1",
                kwargs["dataset"],
                kwargs["class_name"],
                kwargs["shape"],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conf", type=str, default="confs/setup_base_all_PCA_with_pe_v1_shapenet.conf")
    ...
    parser.add_argument("--class_name", type=str, default="unknow", help="class_name")
    ...
    args.expname = "%s_%s" % (args.class_name, args.shape.split(".")[0])
